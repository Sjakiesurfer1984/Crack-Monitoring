...
def render_individual_graphs(df_filtered, selected_channels, start_date_default: date, end_date_default: date, theme: str = "Light",):
    ...
    for idx, ch in enumerate(selected_channels):
        ...
        fig_ch = px.line(
            ...
            y=ch,
            title=ch,
            labels={'datetime': 'Date & Time', ch: 'Crack width difference wrt to first measurement [mm]'},
            template='plotly_dark' if is_dark else 'plotly'
        )

        # Force legend even for single line
        fig_ch.update_traces(name=ch, showlegend=True, line=dict(color=color))
        ...
        fig_ch.update_layout(
            ...
            title={
                ...
                "yanchor": "top",
                "y": 0.95,
                "font": dict(size=18, color='white' if theme == "Dark" else '#111111')
            },
            margin=dict(t=60),
...
def render_combined_normalised_graph(df: pd.DataFrame, all_channel_cols: list[str], theme: str, start_date_default: date, end_date_default: date):
    ...
    fig = px.line(df_melted, x='datetime', y='Value', color='Sensor',
                  ...
                  labels={'datetime': 'Date & Time', 'Value': 'Crack width difference wrt to first measurement [mm]'},
    ...
    **get_plotly_layout(theme),
    ...
    margin=dict(t=60)  # Add space at top for title
)
...
def render_statistics(df: pd.DataFrame, columns: list[str]):
    ...
    if submitted and selected_col:
        ...
        st.markdown(f"**Min. crack width [mm]:** {min_val:.5f}")
        st.markdown(f"**Max. crack width [mm]:** {max_val:.5f}")

        fig_hist = go.Figure(data=[go.Histogram(x=values, nbinsx=nbins)])
        fig_hist.update_layout(
            title=f"Distribution of {selected_col}",
            xaxis_title=selected_col + " [mm]",
            yaxis_title="Count",
