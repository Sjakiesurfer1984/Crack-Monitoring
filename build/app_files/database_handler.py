...
sys.modules['sqlite3.dbapi2'] = apsw
import pandas as pd
import logging
import os
...
class DatabaseHandler:
    ...
    def _connect(self):
        # ensure directory for the DB exists
        parent = os.path.dirname(self.db_path)
        if parent and not os.path.isdir(parent):
            os.makedirs(parent, exist_ok=True)

        try:
            # APSW’s Connection is the entry point
            conn = apsw.Connection(self.db_path)
            logger.info(f"🗄 Connected to APSW database at {self.db_path}")
            return conn
        except Exception as e:
            logger.error(f"❌ Failed to connect via APSW: {e}")
            raise
